...
#import .h5 model
model = tf.keras.models.load_model('model_trained_vg16.h5')
...
async def create_upload_file(file: UploadFile = File(...)):
    ...
    img = Image.open(img_stream).resize((384, 384))

    # img = tf.keras.preprocessing.image.load_img(file_img, target_size=(200, 200))
    img_array = tf.keras.preprocessing.image.img_to_array(img)
    img_array = tf.expand_dims(img_array, 0)
    predictions = model.predict(img_array)
    garbage_types = [ 'battery', 'biological', 'brown-glass', 'cardboard','cloths', 'green-glass','metal', 'paper', 'plastic','shoes' ,'trash', 'white-glass']
